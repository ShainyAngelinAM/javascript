  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        toast.error(error.response?.data?.message || 'Failed to fetch user profile', { position: "top-right" }); // Use Toastify
  ...
  return (
    <Layout style={{ minHeight: '100vh', background: 'linear-gradient(to right, #74ebd5, #ACB6E5)' }}>
      <Header style={{ background: 'transparent', boxShadow: 'none' }}>
        <Title level={2} style={{ color: '#fff', margin: '0', textAlign: 'center' }}>Profile</Title>
      </Header>
      <Content style={{ padding: '20px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        ...
        >
          ...
          <Title level={3} style={{ textAlign: 'center', marginBottom: '10px' }}>User Profile</Title>
          <Row gutter={[0, 16]}>
            <Col span={24}>
      ...
      <Footer style={{ textAlign: 'center', background: 'transparent', color: '#fff' }}>
        &copy; Employee Leave Management System
      </Footer>
      <ToastContainer /> 
    </Layout>
  );
